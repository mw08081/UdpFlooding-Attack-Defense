...
int main() {
    ...
    while (1) {
        ...
        sprintf(message, "c1 's udp packet (%s)", current_time);
        ...
        ssize_t n = recvfrom(sockfd, (char *)buffer, BUFFER_SIZE, MSG_WAITALL, (struct sockaddr *)&other_cliaddr, &len);
        if (n < 0) {
